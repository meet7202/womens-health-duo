...
export function ZalakGroupClassPage() {
  const { classSlug } = useParams<{ classSlug: string }>();
  const content = classSlug
    ? Object.values(ZALAK_GROUP_CLASS_CONTENT).find((item) => item.slug === classSlug)
    : undefined;

  if (!content) {
    return (
      <PageShell breadcrumbs={[{ name: "Home", path: ROUTES.home }]}>
        <SeoHead
          title="Dr. Zalak Shah group classes"
          metaDescription="Explore Dr. Zalak Shah's prenatal, postnatal and Pilates classes."
          path={ROUTES.drZalak}
        />
        <article className="max-w-3xl">
          <h1 className="font-heading text-3xl sm:text-4xl font-semibold text-foreground mb-4">
            Group classes for women's health
          </h1>
          <p className="text-muted-foreground leading-relaxed">
            We could not find that class page. Please choose one of the programme pages below.
          </p>
          <ul className="mt-6 space-y-2">
            {Object.values(ZALAK_GROUP_CLASS_CONTENT).map((item) => (
              <li key={item.key}>
                <Link to={item.path} className="text-primary underline underline-offset-4">
                  {item.title}
                </Link>
              </li>
            ))}
