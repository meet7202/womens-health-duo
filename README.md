...
- **Cloudflare cache purge:** if a new root file (e.g. `/favicon.ico`) still returns **404** right after deploy, purge that URL in **Caching → Purge Cache** — Cloudflare may be serving a cached 404 from before the file existed. Details in [`AGENTS.md`](./AGENTS.md#cloudflare-purge-stale-404-on-new-root-static-files).
